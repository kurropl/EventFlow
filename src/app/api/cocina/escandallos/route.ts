...
export async function GET(request: NextRequest) {
  try {
    ...
    if (tipo === 'evento') {
      ...
      // escandallo); NO se vuelve a multiplicar por guest_count.
      const data = rows.map((r: any) => ({
        ...r,
        recetas: (r.recetas || []).map((line: any) => {
          const h = humanizeUnit(Number(line.cantidad_total || 0), line.unidad || 'ud');
          return { ...line, cantidad_total: h.qty, unidad: h.unit };
        }),
      }));
