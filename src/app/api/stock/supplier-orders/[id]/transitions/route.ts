...
export async function PUT(r: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try { if (!auth(r)) return NextResponse.json({ success: false, error: 'No autorizado' }, { status: 401 });
    ...
    const actual = await querySingle<{status:string}>('SELECT status FROM supplier_orders WHERE id=$1',[id]);
    if (!actual) return NextResponse.json({ success: false, error: 'OC no encontrada' }, { status: 404 });
    if (!validTransitions[actual.status as string]?.includes(nuevo)) return NextResponse.json({ success: false, error: `Transición ${actual.status}→${nuevo} no válida` }, { status: 409 });
