/**
 * EventFlow — Guest by ID API
 * PATCH  /api/guests/[id] — Update guest (RSVP, dietas, grupo…)
...
export async function PATCH(
  ...
  try {
    ...
    const allowed: Record<string, (v: any) => any> = {
      ...
      group_name: (v) => v || null,
      rsvp: (v) => String(v),
      menu_type: (v) => String(v),
      dietary: (v) => JSON.stringify(Array.isArray(v) ? v : []),
      notes: (v) => v ?? null,
    };
...
export async function DELETE(
  ...
  try {
    const { id } = await params;
    await querySingle<any>(`DELETE FROM guests WHERE id = $1 RETURNING id`, [id]);
