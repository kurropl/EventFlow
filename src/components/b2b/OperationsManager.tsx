 ...
 * Incluye mapa de mesas drag & drop in-line (Next.js 14 standalone fix).
 */
...
export default function OperationsManager() {
  ...
  const generateTablesForEvent = useCallback(async (order: EventOrder) => {
    ...
    try {
      ...
      if (data.success && data.data) {
        ...
        // Auto-generate via server API
        const genRes = await fetch('/api/floor-plan/generate', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            guestCount: order.guest_count,
            tablesSuggested: order.tables_suggested,
            kidsCount: order.kids_count,
            waiters: waiters,
          }),
        });
        const genData = await genRes.json();
        if (genData.success) {
          setTables(genData.data);
        }
      }
  ...
  useEffect(() => {
    ...
    const loadWaiters = async () => {
      try {
        const res = await fetch(`/api/event-orders/${selected.id}/waiters`);
        const data = await res.json();
  ...
  const handleMouseDown = useCallback((e: React.MouseEvent, tableId: string) => {
    ...
    setDrag({ tableId, startX: e.clientX, startY: e.clientY, origX: table.x, origY: table.y });
  }, [tables]);
  ...
  const handleResizeStart = useCallback((e: React.MouseEvent, tableId: string) => {
    ...
    const table = tables.find((t) => t.id === tableId);
    if (!table) return;
    const startX = e.clientX, startY = e.clientY, origW = table.width, origH = table.height;
    const handleMove = (me: MouseEvent) => {
      const dx = (me.clientX - startX) / scale, dy = (me.clientY - startY) / scale;
      setTables(prev => prev.map(t => t.id === tableId ? { ...t, width: Math.max(40, snap(origW + dx, GRID_SIZE)), height: Math.max(30, snap(origH + dy, GRID_SIZE)) } : t));
    };
  ...
  const rotateTable = useCallback((tableId: string) => {
    setTables(prev => prev.map(t => t.id === tableId ? { ...t, width: t.height, height: t.width, rotation: (t.rotation + 90) % 360 } : t));
  }, []);
  ...
  const addTable = useCallback(() => {
    ...
    let max = 0;
    tables.forEach(t => { const n = parseInt(t.id.replace(/\D/g, ''), 10); if (n > max) max = n; });
    let nameMax = 0;
    tables.forEach(t => { const m = t.name.match(/\d+/); if (m) { const n = parseInt(m[0], 10); if (n > nameMax) nameMax = n; } });
    const dims = newShape === 'round' ? { width: 60, height: 60 } : newShape === 'rect' ? { width: 80, height: 50 } : { width: 120, height: 40 };
    setTables(prev => [...prev, {
      id: `t${max + 1}`, name: `Mesa ${nameMax + 1}`, x: snap(cx, GRID_SIZE), y: snap(cy, GRID_SIZE),
      ...dims, rotation: 0, capacity: newCapacity, shape: newShape,
      color: TABLE_COLORS[prev.length % TABLE_COLORS.length], waiter: '',
    }]);
    setShowAdd(false);
  ...
  if (selected && viewTab === 'list') {
    ...
    return (<motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <div className="p-4 rounded-xl bg-[#FAF8F5] border border-[#E5E7EB]">
          <p className="text-[10px] text-[#6B7280] uppercase tracking-wide font-semibold">Camareros</p>
        ...
        <div className="p-4 rounded-xl bg-[#FAF8F5] border border-[#E5E7EB]">
          <p className="text-[10px] text-[#6B7280] uppercase tracking-wide font-semibold">Precio</p>
        ...
        <div className="p-4 rounded-xl bg-white border border-[#E5E7EB] space-y-4">
          ...
          {extraItems.map((item, i) => (
            <div key={i} className="flex items-center gap-3">
              <input type="text" value={item.desc} placeholder="Descripción"
                onChange={e => { const copy = [...extraItems]; copy[i] = { ...copy[i], desc: e.target.value }; setExtraItems(copy); }}
              ...
              <input type="number" value={item.amount || ''} placeholder="Importe €"
                onChange={e => { const copy = [...extraItems]; copy[i] = { ...copy[i], amount: +e.target.value }; setExtraItems(copy); }}
          ...
          <div className="pt-2">
            <button onClick={() => setShowComplete(true)}
              ...
              ✅ Marcar Evento como Completado</button>
          </div>
        </div>
      )}
      ...
      <div className="space-y-5">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-xl font-bold text-[#1A1A2E]">
              {selected ? `Mapa — ${selected.client_name}` : 'Mapa de Mesas'}
            </h1>
          ...
          <div className="flex items-center gap-2">
            <button onClick={() => setViewTab('list')}
              className="text-[11px] font-medium px-3 py-1.5 rounded-lg border border-[#E5E7EB] hover:bg-[#F3F4F6] transition-colors">Volver</button>
            {selected && (
              <button onClick={saveLayout} disabled={saving}
                className="text-[11px] font-medium px-3 py-1.5 rounded-lg bg-[#1A1A2E] text-white hover:bg-[#2D2D44] disabled:opacity-60"
        ...
        <div className="flex flex-col lg:flex-row gap-5">
          <div className="flex-1">
            <div ref={containerRef} className="bg-white rounded-2xl border border-[#ECECF1] shadow overflow-hidden"
              ...
              {!selected ? (
                ...
                <div className="flex items-center justify-center h-full text-sm text-[#C9A84C]">
                ...
                <div ref={hallRef} className="relative bg-[#FAFAFC] border-2 border-dashed border-[#E0D3A8] m-4"
                  ...
                  {tables.map((table) => {
                    ...
                    return (
                      <div key={table.id} onMouseDown={(e) => handleMouseDown(e, table.id)}
                        ...
                        <div className="w-full h-full flex items-center justify-center text-white text-[10px] font-semibold overflow-hidden"
                          style={{ backgroundColor: table.color, borderRadius: table.shape === 'round' ? '50%' : table.shape === 'long' ? '8px' : '6px' }}>
                          <div className="text-center">
                            <div>{table.name}</div>
                            <div className="text-[9px] opacity-75">{table.capacity} pax</div>
                            {wObj && <div className="text-[8px] opacity-90 mt-0.5" style={{ color: '#FFF8DC' }}>{wObj.name}</div>}
                          </div>
                        </div>
                        <div className="absolute -bottom-1 -right-1 w-4 h-4 bg-white border-2 border-[#C9A84C] rounded-full cursor-se-resize z-20 opacity-0 group-hover:opacity-100 transition-opacity"
                          onMouseDown={(e) => handleResizeStart(e, table.id)} />
                        {selectedId === table.id && (<>
                          <button onClick={(e) => { e.stopPropagation(); rotateTable(table.id); }}
                            className="absolute -top-2 -left-2 w-5 h-5 bg-white border border-[#C9A84C] rounded-full text-[10px] flex items-center justify-center shadow-sm hover:bg-[#FBF6E9] z-20">↻</button>
                          <button onClick={(e) => { e.stopPropagation(); deleteTable(table.id); }}
                            className="absolute -top-2 -right-2 w-5 h-5 bg-[#FEF2F2] border border-[#DC2626] rounded-full text-[10px] flex items-center justify-center shadow-sm hover:bg-[#FCE3E3] z-20">✕</button>
                        </>)}
                      </div>
                    );
                  })}
                ...
                <div className="space-y-3">
                  <div>
                    ...
                    <input type="text" value={selectedTable.name}
                      onChange={(e) => setTables(prev => prev.map(t => t.id === selectedTable.id ? { ...t, name: e.target.value } : t))}
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <div>
                      ...
                      <input type="number" value={selectedTable.capacity}
                        onChange={(e) => setTables(prev => prev.map(t => t.id === selectedTable.id ? { ...t, capacity: parseInt(e.target.value) || 0 } : t))}
                        className="w-full text-sm border border-[#ECECF1] rounded-xl px-3 py-2 focus:outline-none focus:border-[#C9A84C]" />
                    </div>
                    <div>
                      <label className="block text-[11px] font-medium text-[#6B7280] uppercase tracking-wide mb-1">Forma</label>
                      <select value={selectedTable.shape}
                        onChange={(e) => setTables(prev => prev.map(t => t.id === selectedTable.id ? { ...t, shape: e.target.value as any } : t))}
                        className="w-full text-sm border border-[#ECECF1] rounded-xl px-3 py-2 focus:outline-none focus:border-[#C9A84C]">
                        <option value="round">Redonda</option><option value="rect">Rectangular</option><option value="long">Alargada</option>
                      </select>
                    </div>
                  </div>
                  <div className="grid grid-cols-2 gap-2">
                    <div>
                      ...
                      <input type="number" value={selectedTable.width}
                        onChange={(e) => setTables(prev => prev.map(t => t.id === selectedTable.id ? { ...t, width: parseInt(e.target.value) || 40 } : t))}
                        className="w-full text-sm border border-[#ECECF1] rounded-xl px-3 py-2 focus:outline-none focus:border-[#C9A84C]" />
                    </div>
                    <div>
                      <label className="block text-[11px] font-medium text-[#6B7280] uppercase tracking-wide mb-1">Alto</label>
                      <input type="number" value={selectedTable.height}
                        onChange={(e) => setTables(prev => prev.map(t => t.id === selectedTable.id ? { ...t, height: parseInt(e.target.value) || 30 } : t))}
                        className="w-full text-sm border border-[#ECECF1] rounded-xl px-3 py-2 focus:outline-none focus:border-[#C9A84C]" />
                    </div>
                  </div>
                  <div>
                    ...
                    <select value={selectedTable.waiter || ''}
                      onChange={(e) => setTables(prev => prev.map(t => t.id === selectedTable.id ? { ...t, waiter: e.target.value } : t))}
                      className="w-full text-sm border border-[#ECECF1] rounded-xl px-3 py-2 focus:outline-none focus:border-[#C9A84C]">
                      <option value="">Sin asignar</option>
                      {waiters.map(w => <option key={w.id} value={w.name}>{w.name} ({w.role})</option>)}
                    </select>
                  </div>
                  <div className="flex gap-2 pt-2">
                    <button onClick={() => rotateTable(selectedTable.id)}
                      ...
                      <Icon name="rotateCw" className="w-3 h-3" /> Rotar</button>
                    <button onClick={() => deleteTable(selectedTable.id)}
                      ...
                      <Icon name="trash" className="w-3 h-3" /> Eliminar</button>
                  </div>
                </div>
              </div>
            ) : (
              <div className="bg-white rounded-2xl border border-[#ECECF1] p-6 shadow">
                <p className="text-sm text-[#9CA3AF] text-center">Selecciona una mesa para editar</p>
              </div>
            )}
            ...
            {showBudgetInfo && (
              <div className="bg-[#FBF6E9] rounded-2xl border border-[#E0D3A8] p-4 shadow">
                <h3 className="font-semibold text-sm text-[#1A1A1A] mb-2 flex items-center gap-2">
                  <Icon name="clipboard" className="w-4 h-4" /> Distribución sugerida</h3>
            ...
            {waiters.length > 0 && (
              <div className="bg-white rounded-2xl border border-[#ECECF1] p-4 shadow">
                <h3 className="font-semibold text-sm text-[#1A1A1A] mb-3 flex items-center gap-2">
                  <Icon name="user" className="w-4 h-4" /> Camareros ({waiters.length})</h3>
                <div className="space-y-1.5">
                  {waiters.map(w => {
                    const assigned = tables.filter(t => t.waiter === w.name);
                    return (<div key={w.id} className="flex items-center justify-between text-sm">
                      <div className="flex items-center gap-2">
                        <div className="w-2 h-2 rounded-full bg-[#C9A84C]" />
                        <span className="text-[#1A1A1A]">{w.name}</span>
                        <span className="text-[11px] text-[#9CA3AF]">({w.role})</span>
                      </div>
                      <span className="text-[11px] font-medium text-[#6B7280]">{assigned.length > 0 ? `${assigned.length} mesa${assigned.length > 1 ? 's' : ''}` : '—'}</span>
                    </div>);
                  })}
                </div>
              </div>
          ...
          {showAdd && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
              ...
              <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}
                ...
                <h3 className="font-serif text-lg text-[#1A1A1E]">Añadir mesa</h3>
                <div>
                  ...
                  <div className="flex gap-2">
                    {(['round', 'rect', 'long'] as const).map((s) => (
                      <button key={s} onClick={() => setNewShape(s)}
                        className={`flex-1 text-[12px] py-2 rounded-xl border transition-all ${newShape === s ? 'text-white border-transparent' : 'bg-white text-[#6B7280] border-[#ECECF1]'}`}
                        style={newShape === s ? { background: '#C9A84C' } : {}}>{s === 'round' ? 'Redonda' : s === 'rect' ? 'Rectangular' : 'Alargada'}</button>
                    ))}
                  </div>
                </div>
                <div>
                  <label className="block text-[11px] font-medium text-[#6B7280] uppercase tracking-wide mb-1">Capacidad</label>
                  <input type="number" value={newCapacity} onChange={(e) => setNewCapacity(parseInt(e.target.value) || 8)}
                ...
                <div className="flex gap-3 pt-1">
                  ...
                  <button onClick={addTable}
                    ...
                    style={{ background: 'linear-gradient(135deg, #C9A84C, #A88A3A)' }}>Añadir</button>
                </div>
              </motion.div>
            </motion.div>
          )}
                ...
                <tr key={o.id} onClick={() => handleSelectOrder(o)}
                  ...
                  <td className="px-4 py-3.5 text-center">
                    <span className={`text-[11px] font-medium px-2 py-0.5 rounded-full border ${
                      ...
                      'bg-gray-50 text-gray-500 border-gray-200'
                    }`}>{o.status === 'in_progress' ? 'En curso' : o.status === 'completed' ? '✅' : o.status}</span>
                  </td>
                </tr>
              ))}
            </tbody>
