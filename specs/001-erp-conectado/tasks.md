...
- [x] **T4.1** `events/route.ts` (configurador): crear/vincular `lead` + webhook
  `LEAD_CREATED`. **DoD:** AC4.1 (lead visible en CRM).
- [x] **T4.2** Reparar la sincronización lead↔evento, **doblemente rota**:
...
- [x] **T4.3** `client_token` garantizado por `acceptQuote` (T1.3) → `/invitados/
  [token]` accesible tras aceptación cliente. **DoD:** AC4.2.
- [x] **T4.4** `POST /api/quotes/public/[id]/reject` + botón rechazar en
  `presupuesto/[id]/page.tsx`. **DoD:** AC4.3.
- [x] **T4.5** Mostrar señal/saldo y estado de pago en `presupuesto/[id]` y/o
  `evento/[id]`. **DoD:** AC4.4.
- [x] **T4.6** Decoración por token: ruta pública acotada (`PATCH /api/guest-forms/
  decor` por token) y `invitados/[token]/page.tsx:147` la usa. **DoD:** AC4.5 (sin 401).
- [x] **T4.7** `ClientEventView.tsx:31-38`: mapa de labels alineado con estados
  reales; generar enlace `/evento/[id]` tras aceptación. **DoD:** AC4 (pantalla conectada).

## FASE 5 · Contratos UI cocina (P1 · R5)
- [x] **T5.1** `GET /api/cocina/passes` forma `{success,data}` + `PUT /api/cocina/
  passes/[id]`; `PASS_OPTIONS` desde `service_passes`. **DoD:** AC5.1 (tab Pases
  lista y guarda).
- [x] **T5.2** Alinear tab Hojas: rutas `production|loading|logistics` y forma de
  respuesta (`CocinaPanel.tsx:790-793`). **DoD:** AC5.2 (tab Hojas con datos).
- [x] **T5.3** Unificar forma de `custom_pass_order` y consumidores
